...
```
# Tech Stack: Laundry Shop System
...
* **`routes/`**: ถ้าจะทำฟีเจอร์ใหม่ (เช่น Orders, Customers) ให้สร้างไฟล์ `.ts` แยกไว้ในโฟลเดอร์นี้
* **`index.ts`**: เมื่อสร้าง Route ใหม่เสร็จ ต้องนำมา Import และลงทะเบียนด้วย `app.route()` ที่นี่เสมอ

### 🔹 Frontend (`frontend/src`)

* **`app/pages/`**: สร้างไฟล์หน้าจอใหม่ที่นี่ (เช่น `DashboardPage.tsx`)
* **`App.tsx`**: เพิ่มเส้นทาง URL ของหน้าตัวเองต่อจากหน้า Login
* **`app/utils/api.ts`**: เพิ่มฟังก์ชันเรียก API (Fetch) ต่อจาก `login` ที่นี่เพื่อให้คนอื่นเรียกใช้ได้ง่าย
...
1. **เช็ค `.gitignore` อีกรอบ:** มั่นใจว่าไม่ได้เอา `node_modules` หรือ `dev.db` ขึ้นไป
